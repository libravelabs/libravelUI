...
function useCommandContext() {
    ...
    if (!context)
        throw new Error(
            'useCommandContext must be used within CommandProvider',
        );
    return context;
}
...
function CommandProvider({
    ...
    return (
        <CommandContext.Provider
            value={{ isEmpty, setIsEmpty, items, onAction }}
        >
            {children}
        </CommandContext.Provider>
    );
...
function CommandList({
    ...
    return (
        <Menu
            ...
            className={cn(
                'scrollbar-hidden max-h-96 scroll-py-1 overflow-x-hidden overflow-y-auto p-1',
                className,
            )}
            {...props}
        >
...
function CommandGroup<T extends object>({
    ...
    return (
        ...
        >
            {'title' in props && (
                <Header className="col-span-full my-px block truncate px-2 text-xs font-semibold text-muted-foreground">
                    {props.title}
                </Header>
            )}
...
function CommandItem({
    ...
    return (
        <MenuItem
            ...
            aria-label={tv || 'item'}
            className={cn(
                "relative flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground",
                'transition-all ease-linear hover:bg-accent hover:ps-3 hover:text-accent-foreground',
                props.isDisabled &&
                    'pointer-events-none cursor-not-allowed opacity-50 hover:bg-transparent focus:outline-none',
                tone === 'destructive'
...
function CommandDescription({
    ...
    return (
        <Text
            slot="description"
            className={cn(
                'ms-auto text-base/6 text-pretty text-muted-foreground group-disabled:opacity-50 sm:text-sm/6',
                className,
            )}
            ref={ref}
            {...props}
