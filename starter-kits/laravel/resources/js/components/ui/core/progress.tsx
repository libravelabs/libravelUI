        ...
        >
            {({ percentage, valueText, isIndeterminate }) => (
                <>
                    ...
                    <div
                        className={cn(
                            'relative mt-1 h-2 w-full overflow-hidden rounded-full bg-secondary outline-1 -outline-offset-1 outline-transparent',
                        )}
                    >
                        {!isIndeterminate ? (
                            <motion.div
                                className={cn(
                                    'absolute start-0 top-0 h-full bg-primary',
                                    percentage === 100
                                        ? 'rounded-full'
                                        : 'rounded-l-full',
                                )}
    ...
    return (
        <ProgressBarPrimitive {...props} ref={ref}>
            {({ percentage, isIndeterminate, valueText }) => (
                <div
                    className={cn(
                        'inline-flex min-h-24 min-w-24 flex-col items-center gap-2',
                        className,
                    )}
                >
                    {label && <Label>{label}</Label>}

                    <div className="relative inline-flex aspect-square h-full w-full items-center justify-center">
                        <svg
                            viewBox="0 0 24 24"
                            fill="none"
                            className="block"
                        ...
                        >
                            ...
                            {!isIndeterminate ? (
                                ...
                                <circle
                                    ...
                                    strokeDashoffset={70}
                                    strokeLinecap="round"
                                    className="origin-center animate-[spin_1s_cubic-bezier(0.4,0,0.2,1)_infinite] stroke-primary"
                                />
                            )}
                        </svg>
