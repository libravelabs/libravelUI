...
interface DialogContentProps
    extends
        ...
        Pick<DialogOverlayPrimitiveProps, 'isOpen' | 'onOpenChange'> {
    size?: keyof typeof sizes;
    isBlurred?: boolean;
        ...
        >
            <DialogModal size={size}>
                <DialogContentPrimitive
                    role={role}
                    className={cn(
                        'peer/dialog group/dialog flex w-full flex-col gap-8 rounded-lg border bg-popover p-6 text-popover-foreground shadow-lg duration-200',
                        className,
                    )}
                    {...props}
                >
...
function DialogDescription({
    ...
    return (
        <Text
            slot="description"
            className={cn(
                'text-base/6 text-pretty text-muted-foreground group-disabled:opacity-50',
                className,
            )}
            ref={ref}
            {...props}
...
function DialogBody({ className, ref, ...props }: React.ComponentProps<'div'>) {
    return (
        <div
            ...
            ref={ref}
            className={cn(
                'flex max-h-[calc(var(--visual-viewport-height)-(var(--dialog-header-height)+var(--dialog-footer-height))*2)] flex-col gap-4 overflow-auto',
                className,
            )}
            {...props}
        />
...
function DialogCloseIcon({ className, ...props }: DialogCloseIconProps) {
    return props.isDismissable ? (
        <Button
            ...
            className={cn(
                className,
                'close absolute end-1 top-1 z-50 flex size-8 flex-col place-content-center rounded-xl hover:bg-secondary focus:bg-secondary focus:outline-hidden focus-visible:ring-1 focus-visible:ring-primary sm:end-2 sm:top-2 sm:size-7 sm:rounded-md',
            )}
        >
            <X className="size-4" />
        </Button>
