...
const itemStyles = cva(
    'group relative -mb-px flex cursor-pointer gap-3 border-y px-3 py-2 text-foreground outline-hidden -outline-offset-2 transition select-none first:rounded-t-md first:border-t-0 last:mb-0 last:rounded-b-md last:border-b-0 sm:text-sm',
    {
        variants: {
            isHovered: {
                true: 'bg-muted/50',
            ...
            isSelected: {
                true: 'relative z-20 border-border/50 bg-muted ps-3 before:absolute before:start-0 before:top-0 before:bottom-0 before:w-1 before:bg-primary hover:bg-muted/50',
            },
            isFocused: {
                true: 'outline-hidden',
            },
            isFocusVisible: {
                true: 'bg-muted ring-1 ring-ring outline-hidden hover:bg-muted/50 selected:bg-muted',
            },
            isDisabled: {
...
interface GridListItemProps<T> extends Omit<
    GridListItemPrimitiveProps<T>,
    'selectionMode'
> {
    multiple?: boolean;
    disabled?: boolean;
