...
function ComparatorSlider({ className, children }: ComparatorSliderProps) {
    ...
    return (
        <motion.div
            className={cn(
                '[&>*]:(user‑drag:none) absolute top-0 bottom-0 w-0.5 cursor-ew-resize [&>*]:[-webkit-user-drag:none]',
                !children && 'bg-white',
                className,
            )}
            style={{ left }}
        >
            {children ? (
                ...
                <div className="absolute top-1/2 left-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-white p-1 shadow-md">
