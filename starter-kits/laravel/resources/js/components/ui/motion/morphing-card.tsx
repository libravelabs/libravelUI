...
function MorphingCardContent({
    ...
    return (
        <motion.div
            ...
            layoutId={`card-${uniqueId}`}
            className={cn(
                'pointer-events-auto overflow-hidden bg-popover',
                className,
            )}
            style={style}
            role="dialog"
...
function MorphingCardOverlay({
    ...
    return (
        <motion.div
            ...
            className={cn(
                'fixed inset-0 z-50 h-full w-full bg-black/50 backdrop-blur-sm',
                className,
            )}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
...
function MorphingCardClose({
    ...
    return (
        <motion.button
            ...
            className={cn(
                "absolute top-3 right-3 size-5 cursor-pointer rounded-full bg-background p-1 opacity-60 transition-opacity hover:opacity-100 [&_svg:not([class*='size-'])]:size-3",
                className,
            )}
            initial="initial"
            animate="animate"
