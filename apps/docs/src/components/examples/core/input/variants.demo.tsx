...
const variants: NonNullable<InputProps["variant"]>[] = [
  "default",
  "destructive",
  "ghost",
  "line",
...
export default function VariantsInputDemo() {
  const [variant, setVariant] = useState<InputProps["variant"]>("default");
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-4">
        <div className="space-y-2">
          <Heading size={3}>Variant</Heading>
          <RadioGroup
            value={variant}
            onChange={(v) => setVariant(v as InputProps["variant"])}
      ...
      <VariantsInput variant={variant} size={size} radius={radius} />
    </div>
  );
}
