...
export const starterKits = [
  ...
  },
  {
    ...
    description: "The simple way to start Vite with libravelUI installed.",
    label: "Starter Kit",
  },
];
...
export function StarterKits() {
  return (
    <div className="py-6 sm:py-12">
      <CardHeader className="mb-6 max-w-lg">
        <CardTitle>Starter template</CardTitle>
        <CardDescription>
          A ready-to-use project foundation with essential tools and
          configuration to help you build and launch faster using {app.name}.
        </CardDescription>
      </CardHeader>
      <div className="grid [&>*:nth-last-child(1):nth-child(2n+1)]:lg:col-span-2 [&>*:nth-last-child(1):nth-child(2n)]:lg:col-span-1 gap-2 max-w-xl">
        {starterKits.map((sk) => (
          <Link key={sk.name} href={sk.url} className="decoration-none">
            <Card className="flex flex-col items-center justify-center h-36 *:opacity-40 hover:*:opacity-100 *:transition *:ease-in-out">
              <span className="[&_svg]:size-10">{sk.icon}</span>
              <CardTitle className="font-medium">{sk.name}</CardTitle>
            </Card>
          </Link>
        ))}
      </div>
...
export function IconBrandTanstack(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      data-slot="icon"
...
export function IconBrandVite(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      data-slot="icon"
      viewBox="0 0 40 40"
...
export function BrandLaravelIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
