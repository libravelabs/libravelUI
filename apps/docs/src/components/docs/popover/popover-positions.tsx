...
export function PopoverPositionsBase() {
  return (
...
export const PopoverPositionsCode = `"use client";
...
const placements = ["bottom", "top", "left", "start", "right", "end"];
...
export function PopoverPositions() {
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
      {placements.map((placement, idx) => (
        <Popover key={idx}>
          <PopoverTrigger className="mx-auto" size="sm" variant="outline" radius="md">
            {arrowIcons[placement]}
          </PopoverTrigger>
          <PopoverContent className="p-4 max-w-xs" placement={placement} withArrow>
            Popover shown at <strong>{placement}</strong>.
          </PopoverContent>
        </Popover>
      ))}
