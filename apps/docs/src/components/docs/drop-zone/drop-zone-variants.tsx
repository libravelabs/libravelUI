...
import { DropZone, DropZoneProps } from "@/components/ui/drop-zone";
import { Select } from "@/components/ui/select";

interface DropZoneVariantsBaseProps {
  variant: DropZoneProps["variant"];
  size: DropZoneProps["size"];
  onVariantChange: (v: DropZoneProps["variant"]) => void;
  onSizeChange: (s: DropZoneProps["size"]) => void;
}

export function DropZoneVariantsBase({
  variant,
  size,
  onVariantChange,
  onSizeChange,
}: DropZoneVariantsBaseProps) {
  return (
      ...
      <div className="flex flex-wrap gap-2">
        <Select
          ...
          onSelectionChange={(key) =>
            onVariantChange(key as DropZoneProps["variant"])
          }
        ...
        <Select
          ...
          selectedKey={size}
          onSelectionChange={(key) =>
            onSizeChange(key as DropZoneProps["size"])
          }
          classNames={{ trigger: "min-w-32", content: "min-w-32" }}
