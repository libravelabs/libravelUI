...
```package-install
npm install react-aria-components class-variance-authority clsx tailwind-merge lucide-react
```
  ...
  ],
  {
    variants: {
      ...
      size: {
        ...
        default: "h-9 text-base md:text-sm [&_svg:not([class*='size-'])]:size-4.5",
...
<TypeTable
  type={{
    ...
    isPassword: {
      description: "Whether the input is a password field with toggle visibility.",
      type: "boolean",
      required: false,
    },
    classNames: {
