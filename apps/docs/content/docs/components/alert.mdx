...
```package-install
npm install class-variance-authority lucide-react clsx tailwind-merge motion
```
...
<TypeTable
  type={{
    ...
    children: {
      description: "Optional custom content to replace the default message. Overrides the `message` prop if provided.",
      type: "React.ReactNode",
    },
    open: {
